...
BOOL  dns_do_idna     = TRUE;    /* Convert non-ASCII names to/from ACE */
 ...
 * Returns 1 if okay and if the ip is available,
                          ...
                          WORD qtype, void *addr)
{
   ...
   * turn it into a "Name not found" error.
   */
  dom_errno = DNS_SRV_NAME;
  return (0);
}
...
static int lookup_domain (
       ...
       BOOL         is_ip4,        /* Query A or AAAA record */
       void        *addr)          /* return address */
{
  ...
  size_t len;
  int    rc;
  WORD   qtype = is_ip4 ? DTYPE_A : DTYPE_AAAA;
  ...
  *timedout = TRUE;  /* assume we will timeout */
  ...
  if (add_dom)
  {
    ...
    if (namebuf[dot] != '.')     /* if no trailing dot */
    {
      ...
      {
        strcat (namebuf, ".");
        strcat (namebuf, getpath(loc_domain,1));
      }
    }
    else
...
static const char *client_error (enum DNS_client_code err)
{
  ...
  {
    case DNS_CLI_SYSTEM:
         return (char*) strerror_s (_w32_errno);
 ...
 */
const char *dom_strerror (int err)
{
  static char buf[80];
...
DWORD resolve (const char *name)
{
  ...
  /* If no nameserver or no IP-address, give up now
   */
...
#if defined(USE_IPV6)
int resolve_ip6 (const char *name, void *addr)
{
  struct DNS_query request;
