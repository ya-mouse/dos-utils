...
const struct spi_programmer spi_programmer[] = {
	...
	{NULL}, /* This entry corresponds to SPI_CONTROLLER_INVALID. */
};
...
int default_spi_send_command(unsigned int writecnt, unsigned int readcnt,
			     const unsigned char *writearr, unsigned char *readarr)
{
	struct spi_command cmd[] = { {0}, {0} };
	cmd[0].writecnt = writecnt;
	cmd[0].readcnt  = readcnt;
	cmd[0].writearr = writearr;
	cmd[0].readarr  = readarr;
...
int spi_chip_read(struct flashchip *flash, uint8_t *buf, int start, int len)
{
	 ...
	 */
	if (ffs(flash->total_size * 1024) > (ffs(addrbase) ? ffs(addrbase) : 33)) {
