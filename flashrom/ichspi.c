...
#if defined(__i386__) || defined(__x86_64__) || defined(__DOS__)
...
{
	return mmio_readl((unsigned char *)ich_spibar + X);
}

static uint16_t REGREAD16(int X)
{
	return mmio_readw((unsigned char *)ich_spibar + X);
}

#define REGWRITE32(X,Y) mmio_writel(Y, (unsigned char *)ich_spibar+X)
#define REGWRITE16(X,Y) mmio_writew(Y, (unsigned char *)ich_spibar+X)
#define REGWRITE8(X,Y)  mmio_writeb(Y, (unsigned char *)ich_spibar+X)
...
#define BBAR_MASK	0x00ffff00
	...
	case SPI_CONTROLLER_ICH7:
	case SPI_CONTROLLER_VIA:
		ichspi_bbar = mmio_readl((unsigned char *)ich_spibar + 0x50) & ~BBAR_MASK;
		if (ichspi_bbar)
			msg_pdbg("Reserved bits in BBAR not zero: 0x%04x",
				 ichspi_bbar);
		ichspi_bbar |= minaddr;
		mmio_writel(ichspi_bbar, (unsigned char *)ich_spibar + 0x50);
		ichspi_bbar = mmio_readl((unsigned char *)ich_spibar + 0x50);
		/* We don't have any option except complaining. */
		if (ichspi_bbar != minaddr)
			msg_perr("Setting BBAR failed!\n");
		break;
	case SPI_CONTROLLER_ICH9:
		ichspi_bbar = mmio_readl((unsigned char *)ich_spibar + 0xA0) & ~BBAR_MASK;
		if (ichspi_bbar)
			msg_pdbg("Reserved bits in BBAR not zero: 0x%04x",
				 ichspi_bbar);
		ichspi_bbar |= minaddr;
		mmio_writel(ichspi_bbar, (unsigned char *)ich_spibar + 0xA0);
		ichspi_bbar = mmio_readl((unsigned char *)ich_spibar + 0xA0);
		/* We don't have any option except complaining. */
		if (ichspi_bbar != minaddr)
			msg_perr("Setting BBAR failed!\n");
		break;
...
static void do_ich9_spi_frap(uint32_t frap, int i)
{
	...
	int rwperms = (((ICH_BRWA(frap) >> i) & 1) << 1) |
		      (((ICH_BRRA(frap) >> i) & 1) << 0);
	int offset = 0x54 + i * 4;
	uint32_t freg = mmio_readl((unsigned char *)ich_spibar + offset);
			...
			int ich_generation)
{
	...
	ich_spibar = (unsigned char *)rcrb + spibar_offset;

	switch (spi_controller) {
	case SPI_CONTROLLER_ICH7:
		msg_pdbg("0x00: 0x%04x     (SPIS)\n",
			     mmio_readw((unsigned char *)ich_spibar + 0));
		msg_pdbg("0x02: 0x%04x     (SPIC)\n",
			     mmio_readw((unsigned char *)ich_spibar + 2));
		msg_pdbg("0x04: 0x%08x (SPIA)\n",
			     mmio_readl((unsigned char *)ich_spibar + 4));
		for (i = 0; i < 8; i++) {
			int offs;
			offs = 8 + (i * 8);
			msg_pdbg("0x%02x: 0x%08x (SPID%d)\n", offs,
				     mmio_readl((unsigned char *)ich_spibar + offs), i);
			msg_pdbg("0x%02x: 0x%08x (SPID%d+4)\n", offs + 4,
				     mmio_readl((unsigned char *)ich_spibar + offs + 4), i);
		}
		ichspi_bbar = mmio_readl((unsigned char *)ich_spibar + 0x50);
		msg_pdbg("0x50: 0x%08x (BBAR)\n",
			     ichspi_bbar);
		msg_pdbg("0x54: 0x%04x     (PREOP)\n",
			     mmio_readw((unsigned char *)ich_spibar + 0x54));
		msg_pdbg("0x56: 0x%04x     (OPTYPE)\n",
			     mmio_readw((unsigned char *)ich_spibar + 0x56));
		msg_pdbg("0x58: 0x%08x (OPMENU)\n",
			     mmio_readl((unsigned char *)ich_spibar + 0x58));
		msg_pdbg("0x5c: 0x%08x (OPMENU+4)\n",
			     mmio_readl((unsigned char *)ich_spibar + 0x5c));
		for (i = 0; i < 4; i++) {
			int offs;
			offs = 0x60 + (i * 4);
		...
		msg_pdbg("\n");
		if (mmio_readw((unsigned char *)ich_spibar) & (1 << 15)) {
		...
		break;
	case SPI_CONTROLLER_ICH9:
		tmp2 = mmio_readw((unsigned char *)ich_spibar + 4);
		...
		tmp = mmio_readl((unsigned char *)ich_spibar + 0x50);
		...
		msg_pdbg("0x74: 0x%08x (PR0)\n",
			     mmio_readl((unsigned char *)ich_spibar + 0x74));
		msg_pdbg("0x78: 0x%08x (PR1)\n",
			     mmio_readl((unsigned char *)ich_spibar + 0x78));
		msg_pdbg("0x7C: 0x%08x (PR2)\n",
			     mmio_readl((unsigned char *)ich_spibar + 0x7C));
		msg_pdbg("0x80: 0x%08x (PR3)\n",
			     mmio_readl((unsigned char *)ich_spibar + 0x80));
		msg_pdbg("0x84: 0x%08x (PR4)\n",
			     mmio_readl((unsigned char *)ich_spibar + 0x84));
		msg_pdbg("0x90: 0x%08x (SSFS, SSFC)\n",
			     mmio_readl((unsigned char *)ich_spibar + 0x90));
		msg_pdbg("0x94: 0x%04x     (PREOP)\n",
			     mmio_readw((unsigned char *)ich_spibar + 0x94));
		msg_pdbg("0x96: 0x%04x     (OPTYPE)\n",
			     mmio_readw((unsigned char *)ich_spibar + 0x96));
		msg_pdbg("0x98: 0x%08x (OPMENU)\n",
			     mmio_readl((unsigned char *)ich_spibar + 0x98));
		msg_pdbg("0x9C: 0x%08x (OPMENU+4)\n",
			     mmio_readl((unsigned char *)ich_spibar + 0x9C));
		ichspi_bbar = mmio_readl((unsigned char *)ich_spibar + 0xA0);
		msg_pdbg("0xA0: 0x%08x (BBAR)\n",
			     ichspi_bbar);
		msg_pdbg("0xB0: 0x%08x (FDOC)\n",
			     mmio_readl((unsigned char *)ich_spibar + 0xB0));
		if (tmp2 & (1 << 15)) {
			msg_pinfo("WARNING: SPI Configuration Lockdown activated.\n");
			ichspi_lock = 1;
		}
...
int via_init_spi(struct pci_dev *dev)
{
	...
	msg_pdbg("0x6c: 0x%04x     (CLOCK/DEBUG)\n",
		 mmio_readw((unsigned char *)ich_spibar + 0x6c));
	if (mmio_readw((unsigned char *)ich_spibar) & (1 << 15)) {
		msg_pinfo("WARNING: SPI Configuration Lockdown activated.\n");
		ichspi_lock = 1;
	}
