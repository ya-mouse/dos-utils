...
{
	mcp_gpiostate = mmio_readb((unsigned char *)mcp6x_spibar + 0x530);
	mcp_gpiostate |= 1 << MCP6X_SPI_REQUEST;
	mmio_writeb(mcp_gpiostate, (unsigned char *)mcp6x_spibar + 0x530);

	/* Wait until we are allowed to use the SPI bus. */
	while (!(mmio_readw((unsigned char *)mcp6x_spibar + 0x530) & (1 << MCP6X_SPI_GRANT))) ;
...
int mcp6x_spi_init(int want_spi)
{
	...
	status = mmio_readw((unsigned char *)mcp6x_spibar + 0x530);
