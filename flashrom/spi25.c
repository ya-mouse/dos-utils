...
int spi_chip_erase_60(struct flashchip *flash)
{
	...
	cmds[0].writearr = &writearr[0];
	cmds[1].writearr = &writearr[1];
...
int spi_block_erase_52(struct flashchip *flash, unsigned int addr, unsigned int blocklen)
{
	...
	writearr[1][1] = (addr >> 16) & 0xff;
	writearr[1][2] = (addr >> 8) & 0xff;
	writearr[1][3] = (addr & 0xff);
	cmds[0].writearr = writearr[0];
	cmds[1].writearr = writearr[1];
...
static int spi_write_status_register_ewsr(struct flashchip *flash, int status)
{
	...
	writearr[1][1] = (unsigned char) status;
	cmds[0].writearr = writearr[0];
	cmds[1].writearr = writearr[1];
...
int spi_nbyte_program(int addr, uint8_t *bytes, int len)
{
	...
	unsigned char writearr = JEDEC_WREN;
	struct spi_command cmds[] = {
	{
		JEDEC_WREN_OUTSIZE,
		0,
...
{
	unsigned char cmd[JEDEC_READ_OUTSIZE];
	cmd[0] = JEDEC_READ;
...
int spi_aai_write(struct flashchip *flash, uint8_t *buf, int start, int len)
{
	...
	unsigned char writearr[][6] = {
		{JEDEC_WREN, 0, 0, 0, 0, 0},
		{JEDEC_AAI_WORD_PROGRAM, 0, 0, 0, 0, 0},
	};
	struct spi_command cmds[] = {
	{
		JEDEC_WREN_OUTSIZE,
		...
		JEDEC_AAI_WORD_PROGRAM_OUTSIZE,
		0,
		NULL,
		NULL,
	}, {
	...
	writearr[1][1] = (start >> 16) & 0xff;
	writearr[1][2] = (start >> 8) & 0xff;
	writearr[1][3] = (start & 0xff);
	writearr[1][4] = buf[0];
	writearr[1][5] = buf[1];
	cmds[0].writearr = writearr[0];
	cmds[1].writearr = writearr[1];
	...
	if (start % 2) {
		...
		pos += start % 2;
		writearr[1][0] = JEDEC_AAI_WORD_PROGRAM;
		writearr[1][1] = (pos >> 16) & 0xff;
		writearr[1][2] = (pos >> 8) & 0xff;
		writearr[1][3] = (pos & 0xff);
		writearr[1][4] = buf[pos - start];
		writearr[1][5] = buf[pos - start + 1];
